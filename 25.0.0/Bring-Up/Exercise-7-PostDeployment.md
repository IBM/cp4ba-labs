# Exercise 7: Post Deployment Steps / Deployment Verification
...
3.	Open the **Workloads** section in the menu, and select the **Pods** menu. There should be **58 pods** now in running state, plus some in Completed stages. None should be in Pending or in CrashLoopBackoff state.

    ![58 pods in running state](Images/8.2-47pods.png)		
...
4.  Optionally, for further troubleshooting, use the script [deploymentStatus.sh](LabData/deploymentStatus.sh). By right-clicking the **Raw**  button the script can be saved to a local directory and executed. On a healthy environment it should print following output:
    ...
    ZenStatus: zen operator 6.1.3 build 22:  Completed
...
The following post-deployment steps described here follow the description in the knowledge center under "What to do next": https://www.ibm.com/docs/en/cloud-paks/cp-biz-automation/25.0.0?topic=cpd-option-2-installing-production-deployment-in-openshift-console 
...
19.	In the Openshift Web Console, copy the link to the Content Navigator. There are two, the one with the name *Navigator Login URL for CP4BA* uses the main entry point for the cloud pak, and the one with the name *Navigator Login URL for FNCM* is designed for custom applications and use-cases where Content Navigator is embedded into other applications.Paste the link again into the Private Windows URL entry box (right click and select "Paste and Go")
