...
2.	Run the deployment script, use the project name as parameter to differentiate between deployments in different OCP projects.

    ```
    ./cp4a-deployment.sh -n ibm-cp4ba
    ```
	
3.	Indicate that you accept the license.

4.	Next question is, if in the current OCP cluster you already have a deployment of CP4BA with only the FileNet Component. This would use a custom resource type named `Content`, instead of `ICP4ACluster`. The answer is **No**.
...
7.  Answer the question weather the deployment will use AWS or Azure with **No**. 
...
11.	The script generates the specification file for Cloud Pak 4 Business Automation in the directory `generated-cr/project/<project-name>`. 

12. After completing this step, navigate into the forementioned directory, and review the generated Custom Resource file.

    ```
    cd generated-cr/project/ibm-cp4ba
    ls -l 
    ```
...
14. Before you can apply the specification, it is needed to synchronize the clocks between the worker nodes of the Openshift cluster, find more on this topic in the documentation on
    https://www.ibm.com/docs/en/cloud-paks/cp-biz-automation/25.0.0?topic=cpd-option-1b-deploying-custom-resource-you-created-deployment-script. 
...
Error messages in the logs would have red color, and typically contain keyword "failed", but not all error messages might require followup actions. For monitoring the deployment the best option might be to await the end of the current loop of the Content Operator, and start from reviewing its status, which can easily be viewed from the Openshift Console window. Refer to the [Next Exercise](Exercise-7-PostDeployment.md) for instructions on monitoring the deployment and on required post-deployment activity.
