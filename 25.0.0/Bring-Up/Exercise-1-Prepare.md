...
The preparation of a bastion host is discussed in the documentation on this page: [Preparing a client to connect to the cluster](https://www.ibm.com/docs/en/cloud-paks/cp-biz-automation/25.0.0?topic=deployment-preparing-client-connect-cluster).
...
The OpenShift cluster was configured using OCP version 4.16.54. It will host the PostgreSQL and CP4BA containers which you will install 
...
1. Check that you have an entitlement key with the proper image access. For this, please open [IBM Container Library](https://myibm.ibm.com/products-services/containerlibrary). Log in with your IBM ID. IBMers can use their w3 ID.
...
4.	Scroll down to the **Environments** section. Choose an environment "CP4BA 25.0.0 Bring-Up Lab Environment" with the highest version number. If you have multiple with the highest number, choose the one in the nearest geo. Then click on **Reserve**.
...
12.	Log in with **ocpadmin / passw0rd** (where the third-last character is a zero).

13.	Once logged in, verify that the OpenShift Web Console opens and that you have Administrator access.
 
    ![Admin Access](Images/2.2.3-Admin.png)
	
14. When you check the status of the cluster, the Cluster and the Control Plane are green, but one Operator is degrated. By clicking on the link for **Operators** you can see that the machine-config operator is degraded. The reason is, that Updates of the Machine Config were disabled, before creating the copy of the OCP cluster.
